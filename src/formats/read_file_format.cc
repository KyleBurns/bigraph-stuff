...
auto read_pattern_file_format(const string & format, const string & filename) -> InputGraph
{
    ...
    else if (actual_format == "bigraph")
        return read_pattern_bigraph(move(infile), filename);
...
auto read_target_file_format(const string & format, const string & filename) -> InputGraph
{
    ifstream infile{ filename };
    if (! infile)
        throw GraphFileError{ filename, "unable to open file", false };
    ...
    if (actual_format == "dimacs")
        ...
        return read_target_bigraph(move(infile), filename);
    else if (actual_format == "lad")
        return read_lad(move(infile), filename);
    else if (actual_format == "labelledlad")
        return read_labelled_lad(move(infile), filename);
